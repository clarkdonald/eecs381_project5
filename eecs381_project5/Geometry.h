...
/******************************************************
    This set of simple classes is used to compute
    positions and directions in the plane, 
    in both cartesian and polar coordinates.

    These classes are defined with "struct" to
    make all members public by default.

    These classes make no assumptions about units of
    measurement of distance. Angles
    can be specified in radians, but
    radians can be converted to and from 
    trigonometry degrees in which 0 degrees
    corresponds to (x > 0, y = 0), 
    and 90 degrees corresponds to (x = 0, y > 0).

    Point is a set of (x, y) coordinates. 

    A Cartesian_vector is (delta_x, delta_y) -
    a displacement in Cartesian coordinates.

    A Polar_vector is (r, theta) -
    a displacement in polar coordinates using radians.

    Various overloaded operators support
    computations of positions and directions.
******************************************************/
...
struct Cartesian_vector
{
    ...
    Cartesian_vector(double in_delta_x = 0.,
                     double in_delta_y = 0.) : 
