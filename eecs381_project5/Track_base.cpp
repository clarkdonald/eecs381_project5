...
Track_base::Track_base(Point in_position,
...
Compass_position
Track_base::get_range_and_bearing_from (const Point& p) const
{
...
Compass_position
Track_base::get_range_and_bearing_from (const Track_base * track_ptr) const
{
...
Compass_position
Track_base::get_CPA_range_and_bearing_from(const Track_base *track_ptr) const
{
    double time_to_CPA;
    Compass_position result = 
            compute_CPA(track_ptr->get_course_speed(),
                        course_speed,
                        get_range_and_bearing_from(track_ptr),
                        time_to_CPA);
    return result;
}
...
bool
Track_base::is_inbound_toward(const Track_base * track_ptr) const
{
    bool result = false;
...
void
Track_base::update_position(double time_increment)
{
